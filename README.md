...
🔍 **Type Inference** — Maps values to `TINYINT(1)` / `BIGINT` / `DOUBLE` / `DATETIME` / `TEXT`. ISO 8601, `YYYY-MM-DD HH:MM:SS`, and other common datetime formats are detected automatically. If a later value exceeds the current column width (e.g., a `VARCHAR` or `TEXT` column receives a longer string), the column is automatically widened (`VARCHAR` → `TEXT` → `MEDIUMTEXT` → `LONGTEXT`) without losing existing data, and ingestion continues.
...
📦 **gzip + Recursive Directory** — `.gz` files decompress on the fly. Selecting a folder recursively collects all `.log` and `.gz` files beneath it, including files that share the same name across different sub-folders.
...
| `_te_{col}` | `TEXT` (auto-widened as needed) | Raw value when type coercion fails (added on demand) |
...
🔍 **型推論** — `TINYINT(1)` / `BIGINT` / `DOUBLE` / `DATETIME` / `TEXT` へ自動マッピング。ISO 8601 等の日時文字列も自動判定。後続の値がカラム幅を超えた場合（例：`VARCHAR` や `TEXT` カラムに長い文字列が来た場合）は、既存データを保持したまま自動でカラム幅を拡張（`VARCHAR` → `TEXT` → `MEDIUMTEXT` → `LONGTEXT`）して処理を継続します。
...
📦 **gzip + 再帰ディレクトリ** — `.gz` はオンザフライ解凍。フォルダを指定すると配下の `.log` / `.gz` を再帰取得。異なるサブフォルダに同名ファイルが存在する場合も正しく識別してすべて取り込みます。
...
| `_te_{col}` | `TEXT`（必要に応じ自動拡張） | 型エラー時の生値（自動追加） |
